...
func (lib *KnowledgeLibrary) LoadKnowledgeBaseFromReader(reader io.Reader, overwrite bool) (retKb *KnowledgeBase, retErr error) {
	defer func() {
		if r := recover(); r != nil {
			retKb = nil
			retErr = fmt.Errorf("panic recovered during LoadKnowledgeBaseFromReader, recover \"%v\". send us your report to https://github.com/hartmamt/grule-rule-engine/issues", r)
		}
	}()
